---
locale: ja
translationKey: harusaruhi_album_shingan
title: "心眼"
romanizedTitle: "Shingan"
artist: "春猿火"
type: "スタジオアルバム"
description: "春猿火の正式リリース『心眼』。全曲順を確認し、各楽曲ページへリンクしています。"
releaseDate: "2021-10-06"
label: "℗ KAMITSUBAKI RECORD"
trackCount: 12
duration: "41:09"
categoryTitle: "アルバム"
categorySubtitle: ALBUMS
categoryOrder: 1
itemOrder: 1633503600000
image: /images/albums/harusaruhi/shingan.jpg
officialLinks:
  - label: "公式ディスコグラフィー"
    href: https://kamitsubaki.jp/discography/harusaruhi/824/
  - label: "Apple Music Japan"
    href: https://music.apple.com/jp/album/%E5%BF%83%E7%9C%BC/1688156578?uo=4
tracks:
  -
    number: "1"
    title: "INTRODUCTION -目-"
    duration: "01:37"
    songId: "harusaruhi/originals/INTRODUCTION-目-introduction-eye"
  -
    number: "2"
    title: "逆転"
    duration: "04:17"
    songId: "harusaruhi/originals/逆転-reversal"
  -
    number: "3"
    title: "Lift Up"
    duration: "03:09"
    songId: "harusaruhi/originals/lift-up"
  -
    number: "4"
    title: "猛進"
    duration: "02:33"
    songId: "harusaruhi/originals/猛進-rush"
  -
    number: "5"
    title: "青春"
    duration: "03:30"
    songId: "harusaruhi/originals/青春-youth"
  -
    number: "6"
    title: "告げ口"
    duration: "03:52"
    songId: "harusaruhi/originals/告げ口-the-secret"
  -
    number: "7"
    title: "オオゴト"
    duration: "03:27"
    songId: "harusaruhi/originals/オオゴト-serious-affair"
  -
    number: "8"
    title: "覚醒"
    artist: "春猿火 & SANARI"
    duration: "03:56"
    songId: "harusaruhi/originals/覚醒-awakening-心眼"
  -
    number: "9"
    title: "哀愁さえも仲間"
    duration: "04:21"
    songId: "harusaruhi/originals/哀愁さえも仲間-even-melancholy-companion"
  -
    number: "10"
    title: "台風の眼"
    duration: "04:04"
    songId: "harusaruhi/originals/台風の眼-eye-of-the-storm"
  -
    number: "11"
    title: "居場所"
    duration: "04:46"
    songId: "harusaruhi/originals/居場所-my-place"
  -
    number: "12"
    title: "OUTRODUCTION -眼-"
    duration: "01:37"
    songId: "harusaruhi/originals/OUTRODUCTION-眼-outroduction-eye"
---

## 作品概要

『心眼』は本カタログに収録する正式リリースです。発売日、曲順、収録時間、参加アーティストは公式ディスコグラフィーとApple Music日本版を照合しています。

## トラックとバージョン

全12トラックを掲載。各トラックは個別の楽曲・バージョンページへリンクします。再収録は同一ページを再利用し、ライブ、リミックス、リアレンジ版は別ページとしています。

## 資料方針

ジャケットはApple Musicの最高原生解像度を優先し、フィジカル限定作品は公式商品画像を使用します。

## 出典

- [公式ディスコグラフィー](https://kamitsubaki.jp/discography/harusaruhi/824/)
- [Apple Music](https://music.apple.com/jp/album/%E5%BF%83%E7%9C%BC/1688156578?uo=4)
- [VGMdb 春猿火](https://vgmdb.net/artist/48426)
