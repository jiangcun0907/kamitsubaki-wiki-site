---
locale: ja
translationKey: harusaruhi_album_cream_puff_live
title: "CREAM PUFF LIVE"
romanizedTitle: "CREAM PUFF LIVE"
artist: "春猿火"
type: "ライブアルバム"
description: "春猿火のフィジカル限定公式カバーライブアルバム。公式公開の曲順を完全収録しています。"
releaseDate: "2021-03-17"
label: KAMITSUBAKI RECORD
catalogNumber: "KTR-022"
trackCount: 32
categoryTitle: "アルバム"
categorySubtitle: ALBUMS
categoryOrder: 1
itemOrder: 1615939200000
image: /images/albums/harusaruhi/cream-puff-live.jpg
officialLinks:
  - label: "公式作品ページ"
    href: https://kamitsubaki.jp/discography/harusaruhi/378/
tracks:
  -
    number: "1"
    title: "MC1"
  -
    number: "2"
    title: "Twig"
    songId: "harusaruhi/covers/twig"
  -
    number: "3"
    title: "colors"
    songId: "harusaruhi/covers/colors"
  -
    number: "4"
    title: "MC2"
  -
    number: "5"
    title: "ミュージック"
    songId: "harusaruhi/covers/ミュージック-myuujikku"
  -
    number: "6"
    title: "死んでしまったのだろうか"
    songId: "harusaruhi/covers/死んでしまったのだろうか-shinde-shimatta-no-darou-ka"
  -
    number: "7"
    title: "MC3"
  -
    number: "8"
    title: "最後の晩餐"
    songId: "harusaruhi/covers/最後の晩餐-saigo-no-bansan"
  -
    number: "9"
    title: "君の神様になりたい。"
    songId: "harusaruhi/covers/君の神様になりたい-kimi-no-kamisama-ni-naritai"
  -
    number: "10"
    title: "MC4"
  -
    number: "11"
    title: "ピロウトーク"
    songId: "harusaruhi/covers/ピロウトーク-piroutooku"
  -
    number: "12"
    title: "糸"
    songId: "harusaruhi/covers/糸-ito"
  -
    number: "13"
    title: "CULT"
    songId: "harusaruhi/covers/cult"
  -
    number: "14"
    title: "MC5"
  -
    number: "15"
    title: "腐りきった人生だ。"
    songId: "harusaruhi/covers/腐りきった人生だ-kusarikitta-jinsei-da"
  -
    number: "16"
    title: "春猿火自由律#05「iのアンサー」"
    songId: "harusaruhi/covers/春猿火自由律-05-iのアンサー-harusaruhi-jiyuuritsu-05-i-no-ansaa"
  -
    number: "17"
    title: "MC6"
  -
    number: "18"
    title: "MC7"
  -
    number: "19"
    title: "Mela!"
    songId: "harusaruhi/covers/mela"
  -
    number: "20"
    title: "Caffeine"
    songId: "harusaruhi/covers/caffeine"
  -
    number: "21"
    title: "MC8"
  -
    number: "22"
    title: "CITI"
    songId: "harusaruhi/covers/citi"
  -
    number: "23"
    title: "snow jam"
    songId: "harusaruhi/covers/snow-jam"
  -
    number: "24"
    title: "開戦前夜"
    songId: "harusaruhi/covers/開戦前夜-kaisen-zenya"
  -
    number: "25"
    title: "MC9"
  -
    number: "26"
    title: "たぶん"
    songId: "harusaruhi/covers/たぶん-tabun"
  -
    number: "27"
    title: "MC10"
  -
    number: "28"
    title: "告げ口"
    songId: "harusaruhi/covers/告げ口-tsugeguchi"
  -
    number: "29"
    title: "MC11"
  -
    number: "30"
    title: "ずるいね feat.理芽"
    songId: "harusaruhi/covers/ずるいね-zuruine"
  -
    number: "31"
    title: "MC12"
  -
    number: "32"
    title: "夢の中"
    songId: "harusaruhi/covers/夢の中-yume-no-naka"
---

## 作品概要

『CREAM PUFF LIVE』は本カタログに収録する正式リリースです。発売日、曲順、収録時間、参加アーティストは公式ディスコグラフィーとApple Music日本版を照合しています。

## トラックとバージョン

全32トラックを掲載。MCは曲目表に残し、楽曲ページは作成していません。それ以外の各曲は個別の楽曲・バージョンページへリンクします。再収録は同一ページを再利用し、ライブ、リミックス、リアレンジ版は別ページとしています。

## 資料方針

ジャケットはApple Musicの最高原生解像度を優先し、フィジカル限定作品は公式商品画像を使用します。

## 出典

- [公式作品ページ](https://kamitsubaki.jp/discography/harusaruhi/378/)
- [VGMdb 春猿火](https://vgmdb.net/artist/48426)
