---
locale: ja
translationKey: harusaruhi_album_glitch_stage
title: "GLITCH STAGE"
romanizedTitle: "GLITCH STAGE"
artist: "春猿火"
type: "スタジオアルバム"
description: "春猿火の正式リリース『GLITCH STAGE』。全曲順を確認し、各楽曲ページへリンクしています。"
releaseDate: "2026-06-24"
label: "PHENOMENON RECORD"
trackCount: 7
duration: "25:14"
categoryTitle: "アルバム"
categorySubtitle: ALBUMS
categoryOrder: 1
itemOrder: 1782284400000
image: /images/albums/harusaruhi/glitch-stage.jpg
officialLinks:
  - label: "公式ディスコグラフィー"
    href: https://kamitsubaki.jp/discography/harusaruhi/10753/
  - label: "Apple Music Japan"
    href: https://music.apple.com/jp/album/glitch-stage/6772089272?uo=4
tracks:
  -
    number: "1"
    title: "daydream (Guiano Remix)"
    duration: "03:14"
    songId: "harusaruhi/remixes/daydream-guiano-remix"
  -
    number: "2"
    title: "砂時計 (水槽 Remix)"
    duration: "03:48"
    songId: "harusaruhi/remixes/砂時計(水槽-Remix)-hourglass-suisoh-remix"
  -
    number: "3"
    title: "META (Masayoshi Iimori Remix)"
    duration: "04:11"
    songId: "harusaruhi/remixes/meta-masayoshi-iimori-remix"
  -
    number: "4"
    title: "SWIPE! (YUC'e Remix)"
    duration: "03:06"
    songId: "harusaruhi/remixes/swipe-yuce-remix"
  -
    number: "5"
    title: "声帯学 (椎乃味醂 Remix)"
    duration: "03:04"
    songId: "harusaruhi/remixes/声帯学(椎乃味醂-Remix)-vocal-cordology-sheeno-mirin-remix"
  -
    number: "6"
    title: "ディストーション (Ava1anche Remix)"
    duration: "04:20"
    songId: "harusaruhi/remixes/ディストーション(Ava1anche-Remix)-distortion-ava1anche-remix"
  -
    number: "7"
    title: "Deep Invite (TeddyLoid Remix)"
    duration: "03:31"
    songId: "harusaruhi/remixes/deep-invite-teddyloid-remix"
---

## 作品概要

『GLITCH STAGE』は本カタログに収録する正式リリースです。発売日、曲順、収録時間、参加アーティストは公式ディスコグラフィーとApple Music日本版を照合しています。

## トラックとバージョン

全7トラックを掲載。各トラックは個別の楽曲・バージョンページへリンクします。再収録は同一ページを再利用し、ライブ、リミックス、リアレンジ版は別ページとしています。

## 資料方針

ジャケットはApple Musicの最高原生解像度を優先し、フィジカル限定作品は公式商品画像を使用します。

## 出典

- [公式ディスコグラフィー](https://kamitsubaki.jp/discography/harusaruhi/10753/)
- [Apple Music](https://music.apple.com/jp/album/glitch-stage/6772089272?uo=4)
- [VGMdb 春猿火](https://vgmdb.net/artist/48426)
