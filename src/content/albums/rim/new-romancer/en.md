---
locale: en
translationKey: rim_album_new_romancer
title: "NEW ROMANCER"
romanizedTitle: "NEW ROMANCER"
artist: "RIM"
type: "Studio album"
description: "NEW ROMANCER, an official RIM release with a verified track list and song links."
releaseDate: "2021-07-21"
label: "℗ KAMITSUBAKI RECORD"
trackCount: 14
duration: "49:15"
categoryTitle: "Albums"
categorySubtitle: ALBUMS
categoryOrder: 1
itemOrder: 1626850800000
image: /images/albums/rim/new-romancer.jpg
officialLinks:
  - label: "Official discography"
    href: https://kamitsubaki.jp/discography/rim/703/
  - label: "Apple Music Japan"
    href: https://music.apple.com/jp/album/new-romancer/1688521731?uo=4
tracks:
  -
    number: "1"
    title: "ピロウトーク"
    duration: "03:42"
    songId: "rim/originals/ピロウトーク-pillow-talk"
  -
    number: "2"
    title: "ラヴソング"
    duration: "03:44"
    songId: "rim/originals/ラヴソング-love-song"
  -
    number: "3"
    title: "さみしいひと"
    duration: "03:02"
    songId: "rim/originals/さみしいひと-lonely"
  -
    number: "4"
    title: "宿木"
    duration: "03:04"
    songId: "rim/originals/宿木-mistletoe"
  -
    number: "5"
    title: "いたいよ"
    duration: "03:16"
    songId: "rim/originals/いたいよ-it-hurts"
  -
    number: "6"
    title: "甘美な無法"
    duration: "04:13"
    songId: "rim/originals/甘美な無法-luscious-lawless"
  -
    number: "7"
    title: "食虫植物"
    duration: "02:38"
    songId: "rim/originals/食虫植物-carnivorous-plant"
  -
    number: "8"
    title: "胎児に月はキスをしない"
    duration: "02:28"
    songId: "rim/originals/胎児に月はキスをしない-the-moon-not-kiss-the-foetation"
  -
    number: "9"
    title: "NEUROMANCE"
    duration: "03:57"
    songId: "rim/originals/neuromance"
  -
    number: "10"
    title: "ユーエンミー"
    duration: "03:55"
    songId: "rim/originals/ユーエンミー-you-and-me"
  -
    number: "11"
    title: "クライベイビー"
    duration: "03:00"
    songId: "rim/originals/クライベイビー-cry-baby"
  -
    number: "12"
    title: "魔的 (feat. KAF)"
    duration: "03:41"
    songId: "rim/collaborations/魔的-magical"
  -
    number: "13"
    title: "やさしくしないで"
    duration: "04:27"
    songId: "rim/originals/やさしくしないで-dont-be-kind-to-me"
  -
    number: "14"
    title: "十九月"
    duration: "04:07"
    songId: "rim/originals/十九月-juuku-gatsu"
---

## Overview

NEW ROMANCER is an official release represented in this catalog. Release date, track order, durations, and credited performers were cross-checked against the official discography and Apple Music Japan.

## Tracks and versions

This page lists 14 tracks. Every track links to a dedicated song or version entry. Reissues reuse one entry, while live, remix, and rearranged versions remain distinct.

## Media policy

Artwork uses the highest native Apple Music asset available; physical-only releases use the official product image.

## Sources

- [Official discography](https://kamitsubaki.jp/discography/rim/703/)
- [Apple Music](https://music.apple.com/jp/album/new-romancer/1688521731?uo=4)
- [VGMdb RIM](https://vgmdb.net/artist/53598)
